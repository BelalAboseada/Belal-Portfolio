  ...
  return (
    <Component className={cn("h-[2ch] w-fit overflow-y-hidden select-none max-md:h-5", className)} {...props}>
      <a onClick={handleClick} href={url} className="group cursor-pointer">
        <span
          className={cn(
            "block font-fancy -translate-y-0 transition-all duration-300 ease-in-out will-change-auto group-hover:translate-y-[-100%]",
            { flex: icon }
          )}
        >
          {icon && <SvgIcon />}
        ...
        <span
          className={cn(
            "block font-fancy transition-all duration-300 ease-in-out will-change-auto group-hover:translate-y-[-100%]",
            { flex: icon }
          )}
        >
          {icon && <SvgIcon />}
